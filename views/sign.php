...
<head>
    ...
    <title>{{ $document->name }} | Sign documents online</title>
    <!-- Ion icons -->
...
<body>
    ...
    <div class="content">
        <div class="page-title" style="overflow:visible;">
            <div class="pull-right page-actions">
                ...
                <button class="btn btn-danger send-to-server-click"  data="requestid:{{ $request->id }}|file:{{ $document->id }}|csrf-token:{{ csrf_token() }}" url="<?=url("Request@decline");?>" warning-title="Are you sure?" warning-message="This request will be declined and email sent to sender." warning-button="Yes, Decline" loader="true"><i class="ion-ios-close-outline"></i> Decline </button>
                @else
                @if ( in_array("edit",json_decode($user->permissions)) )
                @if ( $document->extension != "pdf" )
                <button class="btn btn-success btn-responsive send-to-server-click"  data="source:viewer|document_key:{{ $document->document_key }}|csrf-token:{{ csrf_token() }}" url="<?=url("Document@convert");?>" warning-title="Convert?" warning-message="This file should be converted to PDF before signing or editing. Click convert to proceed." warning-button="Convert" loader="true"><i class="ion-edit"></i>{{ $lauchLabel }}</button>
                @else
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu document-menu">
                        ...
                        @endif
                        @if ( $document->extension != "pdf" )
                        <li data-toggle="tooltip" data-placement="left" data-original-title="Convert To PDF"><a href="" class="send-to-server-click"  data="document_key:{{ $document->document_key }}|csrf-token:<?=csrf_token();?>" url="<?=url("Document@convert");?>" warning-title="Are you sure?" warning-message="This file will be converted to PDF and this action is irreversable." warning-button="Continue" loader="true"><i class="ion-usb"></i><span> Convert</span></a></li>
                        <li class="divider"></li>
                        @endif
                        @if ( in_array("delete",json_decode($user->permissions)) )
                        <li><a href="" class="send-to-server-click"  data="source:viewer|file:{{ $document->id }}|csrf-token:{{ csrf_token() }}" url="<?=url("Document@deletefile");?>" warning-title="Are you sure?" warning-message="This file will be deleted and can't be recovered." warning-button="Continue" loader="true"><i class="ion-ios-trash-outline"></i> <span>Delete</span></a></li>
                        @endif
                    </ul>
                </div>
               @endif
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <input type="text" id="foo" class="form-control" value="{{ env('APP_URL') }}/view/{{ $document->document_key }}" placeholder="Sharing link" readonly="readonly">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="simcy-form"action="<?=url("Document@replace");?>" data-parsley-validate="" loader="true" method="POST">
                    <div class="modal-body">
                        <p>Replacing a file will keep all the document history and settings. Only PDF<?php if(env("ALLOW_NON_PDF") == "Enabled"){ ?>, Word, Excel and Power Point <?php } ?> allowed.</p>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="simcy-front-form" callback="updateRequestFields()" data-parsley-validate="" loader="true" method="POST">
                    <div class="modal-body">
                        <p>The sender requested some fields from you.</p>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="simcy-front-form" callback="addField()" data-parsley-validate="" loader="true" method="POST">
                    <div class="modal-body">
                        <p>Add Custom fields that you use frequently.</p>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Field Label/Name</label>
                                    <input type="text" name="fieldlabel" class="form-control" placeholder="i.e City" data-parsley-required="true">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="fieldvalue" class="form-control" placeholder="i.e New York" data-parsley-required="true">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="addInputField" role="dialog">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Add Input Field</h4>
                </div>
                <form class="simcy-front-form" callback="addInputField()" data-parsley-validate="" loader="true" method="POST">
                    <div class="modal-body">
                        <p>Add an input field that will be filled by users when they receive a signing request.</p>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Input Field Label/Name</label>
                                    <input type="text" name="inputfieldlabel" class="form-control" placeholder="i.e Full Name" data-parsley-required="true">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label><input type="checkbox" class="switch" name="savefield" value="save" checked /> Save this input field.</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Add Input Field</button>
                    </div>
                </form>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="simcy-front-form" callback="selectDocImage()" data-parsley-validate="">
                    <div class="modal-body">
                        <p>Select an image to add to your document.</p>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Choose file</label>
    ...
    <div class="modal fade" id="sendRequest" role="dialog">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Signature Request</h4>
                </div>
                <form class="simcy-front-form" callback="validateRequest()" data-parsley-validate="" loader="false" method="POST">
                    <div class="modal-body">
                        ...
                        <p>A copy of this template will be created on the documents folder for this signing request. If there are any fields the recipient will be asked to fill.</p>
                        @endif
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Message to recipients</label>
                                    <textarea class="form-control" name="requestmessage" rows="3" required="" placeholder="Message to recipients"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <p class="text-muted">Enter email addresses of the recipients separated by a comma.</p>
                        ...
                        <div class="form-group duplicate-request" style="display: none;">
                            <div class="row">
                                <div class="col-md-12">
                                    <label>
                                        <input type="checkbox" class="switch" name="duplicate" value="1" /> Send separate copies for each request
                                    </label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group restricted-request">
                            <div class="row">
                                <div class="col-md-12">
                                    <label>
                                        <input type="checkbox" class="switch" name="restricted" value="1" /> Receiver to sign specific points or add inputs.
                                    </label> 
                                </div>
                            </div>
                        </div>
                             ...
                             <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="select-from-tabs">
                                        <ul class="tabs">
                                            <li class="active"><a data-toggle="tab" href="#send-team">Team/Staff</a></li>
                                            <li><a data-toggle="tab" href="#send-customers">Customers </a></li>
                                        </ul>
                                        <div class="tab-content">
                                            <div id="send-team" class="tab-pane fade in active">
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        ...
                                                        @endforeach
                                                        @else
                                                            <p class="text-center">It's empty here!</p>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Message to recipients</label>
                                    <textarea class="form-control" name="message" rows="3" required="" placeholder="Message to recipients"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <p class="text-muted">Enter email addresses of the recipients separated by a comma.</p>
                                    <input type="text" name="receivers" class="form-control" placeholder="Enter emails"  data-role="tagsinput" data-parsley-required="true">
                                    <p></p>
                                    <input type="hidden" name="document_key" value="{{ $document->document_key }}" />
                                    <input type="hidden" name="csrf-token" value="<?=csrf_token();?>" />
                                </div>
    ...
    <div class="modal fade" id="unlockFile" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Protected File</h4>
                </div>
                <form class="unlock-file"action="" data-parsley-validate="" loader="true" method="POST">
                    ...
                    <div class="alert alert-danger alert-dismissable text-center document-password-error" style="display: none;">
                        ...
                        <p class="text-muted">This file is password protected. Enter the correct password to unlock.</p>
                        <div class="form-group">
                             <div class="row">
                                <div class="col-md-12">
                                    <label>Document Password</label>
                                    <input type="password" class="form-control" name="docpassword" placeholder="Document Password" data-parsley-required="true">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <a href="<?=url("Document@get");?>" class="btn btn-default">Close</a>
                        <button class="btn btn-primary" type="submit">Unlock File</button>
                    </div>
                </form>
            </div>
      ...
      <div class="modal fade" id="protectFile" role="dialog">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Document Protection</h4>
            </div>
              <form class="simcy-form"action="<?=url("Document@protect");?>" data-parsley-validate="" loader="true" method="POST">
                    <div class="modal-body">
                        <div class="alert alert-warning">
                          <strong>Head up!</strong> This action is irreversible and this document will nolonger be signed or modified by signer after protection is set.
                        </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch" name="permission[]" value="print"/> Prevent document printing.</label> 
                              </div>
                            </div>
                          </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch" name="permission[]" value="copy"/> Prevent copying of text and images from this document.</label> 
                              </div>
                            </div>
                          </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch" name="permission[]" value="modify"/> Prevent document modification.</label> 
                              </div>
                            </div>
                          </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch" name="permission[]" value="extract"/> Prevent content extraction from document.</label> 
                              </div>
                            </div>
                          </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch" name="permission[]" value="assemble"/> Prevent document sssembling.</label> <span class="text-muted text-xs"> insert, rotate, or delete pages and create bookmarks or thumbnail images</span>
                              </div>
                            </div>
                          </div>
                          <div class="form-group">
                            <div class="row">
                              <div class="col-md-12">
                                <label><input type="checkbox" class="switch password-protect-toggle" name="setPassword" value="Yes" /> Set a password for this document.</label> 
                              </div>
                            </div>
                          </div>
                          <div class="form-group protection-password" style="display: none;">
                            <div class="row">
                              <div class="col-md-12">
                                    <label>Enter user password</label><span class="text-muted text-xs"> For other users/public to view file</span>
                ...
                @foreach ( $chats as $chat )
                    ...
                    @endif
                            @if ( !empty($chat->avatar) )
                                <img class='chat-image chat-image-default' src='<?=url("")?>uploads/avatar/{{ $chat->avatar }}'  data-toggle="tooltip" data-placement="top" title="{{ isset($chat->fname) ? $chat->fname : 'Unknown' }} {{ isset($chat->lname) ? $chat->lname : 'User' }}" />
                            @else
                                <img class='chat-image chat-image-default' src='<?=url("")?>assets/images/avatar.png'  data-toggle="tooltip" data-placement="top" title="{{ isset($chat->fname) ? $chat->fname : 'Unknown' }} {{ isset($chat->lname) ? $chat->lname : 'User' }}" />
                            @endif
       ...
       <div class="right-bar-body">
        <div class="field-list">
            ...
            <div class="field-item">
                ...
                <span class="text-muted text-xs">Company</span>
            </div>
            @endif
            <div class="field-item">
                <div>{{ mb_substr($user->fname, 0, 1, 'utf-8') }} {{ mb_substr($user->lname, 0, 1, 'utf-8') }}</div>
                <span class="text-muted text-xs">Initial</span>
            </div>
            <div class="field-item">
                <div>{{ $user->email }}</div>
            ...
            <div class="field-item">
                <div>{{ date("F j, Y") }}</div>
                <span class="text-muted text-xs">Today's Date</span>
            </div>
            @if ( !empty($fields) )
