...
class Database {
    ...
    protected function addConditions(array $pairs, $comparator, $operator) {
        foreach($pairs as $field => $value) {
            ...
            if ($value === null) {
                if ($comparator === '=') {
                    array_push($this->conditions, "{$operator} `{$field}` IS NULL");
                } elseif ($comparator === '!=') {
                    array_push($this->conditions, "{$operator} `{$field}` IS NOT NULL");
                } else {
                    array_push($this->conditions, "{$operator} `{$field}` {$comparator} NULL");
                }
            } elseif ($comparator == "IN" || $comparator == "NOT IN") {
    ...
    protected function addFields(array $fields) {
        foreach($fields as $field) {
            // Don't wrap * wildcard or SQL functions in backticks
            if ($field === '*' || stripos($field, '(') !== false) {
                // This is a wildcard or function call like COUNT(), SUM(), etc.
    ...
    public function get() {
        ...
        return $this->getResults("SELECT {$this->getFields()} FROM {$this->tableName} WHERE {$this->getConditions()}{$this->order}");
    }
